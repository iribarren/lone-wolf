...
| FR-014 | The app MUST always surface the campaign's current stage together with suggested next actions derived from the system's flow (the pacing prompts). | US2 | T117 | `backend/tests/Unit/Campaigns/FlowEngineTest.php` (guidance offers advance actions derived from the outgoing edges; a terminal stage yields a conclude action instead)<br>`frontend/tests/components/campaign/AdvanceActions.test.tsx` (one control per suggested action; conclude routes separately)<br>`frontend/tests/components/campaign/StagePanel.test.tsx`<br>`frontend/tests/e2e/play.spec.ts`<br>`scripts/check-contract.sh` (gate C validates every suggested action against the contract's SuggestedAction on each run) | `backend/src/Campaigns/Domain/FlowEngine.php`<br>`backend/src/Campaigns/Application/GetCampaignStateHandler.php`<br>`backend/src/Campaigns/Application/Query/StageView.php`<br>`frontend/src/components/campaign/AdvanceActions.tsx` | COVERED | ✅ Met |
...
| FR-016 | Stage advancement MUST only follow transitions the system's flow permits; illegal moves MUST be refused with an explanation of legal alternatives. | US2 | T039, T116, T118 | `backend/tests/Unit/Campaigns/FlowEngineTest.php` (8 cases: legal successors only, refusal carries every legal alternative, terminal stages explain there are none)<br>`backend/features/campaigns/guided_play.feature` (an illegal move is refused over HTTP)<br>`frontend/tests/components/campaign/AdvanceActions.test.tsx` (refusal feedback shows the legal alternatives)<br>`scripts/check-contract.sh` (gate C asserts the advance answers its documented status with a conformant state, and that the 422 refusal carries a non-empty `legalAlternatives`) | `backend/src/Campaigns/Domain/FlowEngine.php`<br>`backend/src/Campaigns/Domain/IllegalStageTransitionException.php`<br>`backend/src/Campaigns/Infrastructure/Api/EventListener/PlayRefusalExceptionListener.php` | COVERED | ✅ Met |
...
| FR-021 | Players MUST be able to create PCs and NPCs belonging to a specific campaign. | US5 | T119 | `backend/features/characters/sheets.feature` (a conforming PC is accepted; the lighter NPC set passes; an NPC with no attributes still answers an attributes object)<br>`frontend/tests/components/characters/CharacterForm.test.tsx` (a labelled control per field; the kind control appears on create only)<br>`frontend/tests/e2e/characters.spec.ts` (a player creates a character and edits it) | `backend/src/Characters/Application/CreateCharacterHandler.php`<br>`backend/src/Characters/Domain/CharacterKind.php`<br>`frontend/src/components/characters/CharacterForm.tsx` | COVERED | 🟨 Met (API only) — **⚠ DISAGREES** (§4.6) |
| FR-022 | Each system MUST define the expected structure of its character sheets, and every character MUST conform to the structure of its campaign's system. | US5 | T119 | `backend/tests/Unit/Rulesets/SheetStructureTest.php` (unique keys, select options required, version stamp)<br>`backend/tests/Unit/Characters/AttributeValidatorTest.php` (conforming attributes pass, unknown keys are rejected)<br>`frontend/tests/components/characters/CharacterPanel.test.tsx` (two differently shaped sheets render with no hardcoded field list)<br>`backend/features/characters/sheets.feature` (a character with no attributes still conforms to the sheet contract)<br>`scripts/check-contract.sh` (gate C validates every character payload against CharacterView) | `backend/src/Rulesets/Domain/SheetStructure.php`<br>`backend/src/Characters/Domain/SheetSchema.php`<br>`backend/src/Characters/Domain/AttributeValidator.php`<br>`backend/src/Characters/Infrastructure/Persistence/DoctrineSheetStructureProvider.php` | COVERED | ✅ Met |
| FR-023 | Character submissions that do not conform MUST be rejected with field-level guidance identifying what is wrong. | US5 | T075, T116 | `backend/tests/Unit/Characters/AttributeValidatorTest.php` (every violation carries a field and a message)<br>`backend/features/characters/sheets.feature` (missing and wrong-typed fields are refused field-level)<br>`frontend/tests/components/characters/CharacterForm.test.tsx` (each violation shows against its own field; a refusal naming no field gets its own region)<br>`frontend/tests/e2e/characters.spec.ts`<br>`scripts/check-contract.sh` (gate C asserts the 422 carries a non-empty `violations` of `{field, message}`) | `backend/src/Characters/Domain/AttributeViolation.php`<br>`backend/src/Characters/Domain/SheetValidationException.php`<br>`backend/src/Characters/Infrastructure/Api/EventListener/SheetValidationProblemListener.php`<br>`frontend/src/components/characters/CharacterForm.tsx` | COVERED | 🟨 Met (API only) — **⚠ DISAGREES** (§4.6) |
...
| FR-027 | Invalid dice notation MUST be refused with a message identifying the problem; no partial or misleading result may be produced. | US6 | T116 | `backend/tests/Unit/Dice/DiceNotationParserTest.php` (malformed, `invalid_count`, `invalid_faces`, out-of-bounds each get their own reason, pre-roll)<br>`backend/features/dice/notation.feature` (pathological input refused pre-roll with a specific reason)<br>`frontend/tests/components/dice/DiceRollerWidget.test.tsx` (each reason renders its own notice and never a result — not even a stale one)<br>`scripts/check-contract.sh` (gate C asserts the 422 names a typed `reason` from the contract's enum) | `backend/src/Dice/Domain/DiceNotationFailureReason.php`<br>`backend/src/Dice/Domain/InvalidDiceNotationException.php`<br>`backend/src/Dice/Infrastructure/Api/EventListener/DiceNotationProblemListener.php`<br>`frontend/src/components/dice/DiceRollerWidget.tsx` | COVERED | ✅ Met |
...
| FR-029 | Players MUST be able to log a roll (notation, dice values, total, timestamp) into the campaign journal. | US6 | T090, T116 | `backend/tests/Unit/Dice/DiceHandlersTest.php` (the log stamps the current stage and persists the snapshot; invalid notation persists nothing)<br>`backend/features/dice/notation.feature` (the log action appends the roll **and answers with both payloads embedded**)<br>`scripts/check-contract.sh` (gate C fails the moment either payload comes back as an IRI string)<br>`frontend/tests/components/dice/DiceRollerWidget.test.tsx` (logging confirms; a malformed result never crashes the widget) | `backend/src/Dice/Application/RollAndLogHandler.php`<br>`backend/src/Dice/Infrastructure/Api/Processor/RollAndLogProcessor.php`<br>`backend/src/Dice/Infrastructure/Api/DiceRollResource.php`<br>`backend/src/Journal/Domain/RollSnapshot.php` | COVERED | 🟧 Partial — **⚠ DISAGREES** (§4.7) |
...
|---|---|---|
| `COVERED` | 26 | FR-002, FR-003, FR-004, FR-005, FR-006, FR-007, FR-008, FR-009, FR-011, FR-012, FR-013, FR-014, FR-015, FR-016, FR-018, FR-019, FR-020, FR-021, FR-022, FR-023, FR-024, FR-025, FR-027, FR-029, FR-030, FR-031 |
| `NO-TASK` | 4 | FR-001, FR-017, FR-026, FR-028 |
| `NO-TEST` | 1 | FR-010 |
| `NOT-IMPLEMENTED` | 0 | — |
| **Total** | **31** | |
...
3. **FR-017** — `NO-TASK`. Delivered by `T046`/`T052` and the pagination increment.
4. **FR-026** — `NO-TASK`. Delivered by `T086`/`T089`.
5. **FR-028** — `NO-TASK`. Delivered by `T087`/`T089`/`T092`.

The four `NO-TASK` rows are a citation gap, not a delivery gap: every one is implemented and
...
**Updated 2026-09-02.** Phase 11's convergence closed four of the eight citation gaps: `T116`
names FR-016/FR-023/FR-027/FR-029, `T117` names FR-014, `T118` names FR-016 and `T119` names
FR-021/FR-022, so FR-014, FR-021, FR-022 and FR-027 move from `NO-TASK` to `COVERED`. The same
phase added `scripts/check-contract.sh` to seven Test cells: gate C is the only thing that proves
these requirements hold in the *response payload* rather than in the declaration.
