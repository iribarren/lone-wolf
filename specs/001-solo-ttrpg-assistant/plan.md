...
| Contract-First Decoupled API | V | PASS (by design) | Single source of truth: OpenAPI contract in `contracts/` (authored first, verified against API Platform's generated spec at runtime). Player frontend consumes generated client only — no DB access, no session sharing, no server-side templating across that boundary. The EasyAdmin backoffice is server-rendered and session-authenticated under the Principle V backoffice exemption (Constitution 1.1.0), governed by `contracts/admin-backoffice.md`. Breaking changes require versioned migration path. |
